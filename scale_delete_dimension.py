...
# ファイルの確認
# PCA
dir_path_PCA = 'posture_png/pca_png/ar_sc_pca_png'

if os.path.exists(dir_path_PCA):
    shutil.rmtree(dir_path_PCA)
os.mkdir(dir_path_PCA)

p_file = open('ar_sc_pca_contribution_rate.txt', 'w')


# SVD
dir_path_SVD = 'posture_png/svd_png/sc_svd_png'
if os.path.exists(dir_path_SVD):
    shutil.rmtree(dir_path_SVD)
os.mkdir(dir_path_SVD)

s_file = open('sc_svd_contribution_rate.txt', 'w')
...
for posture in range(posture_num):
    ...
    df_svd = pd.DataFrame(svd.fit_transform(df_std))
